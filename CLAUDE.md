...
- **run-loop** — ループ本体。triage→案件1件を explorer→implementer→reviewer→PR まで進める。手動 `/run-loop`、定期実行 `/loop 30m /run-loop`。
- **triage** — CI/issue/直近コミットを読み、対応項目を `loop/progress.md` に追記する（記憶更新のみ。実装はしない）。
...
無人で回るループは無人で間違うループでもある。最終確認を制度として残す:

1. **ループは自動マージしない。** PR 作成までが自動の上限。マージは人間操作でのみ起きる。
2. ループが自信を持てなかった／reviewer が指摘を残した案件は、必ず `loop/progress.md` の **Backlog / Triage** に上げる（人間に見える形で残す）。
3. **週次メンテ**: `git worktree prune` 相当の後片付けと、`CLAUDE.md`／各 `SKILL.md` の陳腐化レビュー（古い指示は無害ではなく有害）。
4. 検証役の "done" は主張であって証明ではない。マージ可否の判断を自動化しない。トークンは「第二の意見に金を払う価値がある場所」にだけ使う。
